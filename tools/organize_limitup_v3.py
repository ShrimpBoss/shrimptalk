...
def parse_date_from_content(content, filename):
    ...
    if match:
        ...
        file_match = re.search(r'(\d+)', os.path.basename(filename))
        if file_match:
            num = int(file_match.group(1))
            year = 2026 if num <= 40 else 2025
        else:
            year = 2025  # 默认 2025 年
...
def main():
    ...
    # 简单统计 2025 年
    data_2025_count = 0
    for f in files_2025[:10]:  # 只测试前 10 个
        try:
            with open(f, 'r', encoding='utf-8') as file:
                content = file.read()
            date = parse_date_from_content(content, f)
            if date and date.year == 2025:
                data_2025_count += 1
        except:
            pass
    
    print(f"\n2025 年文件测试解析到 {data_2025_count}/{len(files_2025[:10])} 个有效日期")
